...
export class UIElement extends Reporter(ElementBase(HTMLElement)) {
     ...
     *  - exec event handler attached with 'elem.on<event> = fn', no param but 'this' is the element
     *  - if no 'elem.on' handler get on<event> attribute from the element and exec it, 'this' is the element
     * @param evtname
     * @param detail
     */
    emit(evtname, detail) {
